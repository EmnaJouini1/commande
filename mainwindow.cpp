...
void MainWindow::on_lineEdit_recherche_cursorPositionChanged() {
    ...
    QSqlQuery query;
    query.prepare("SELECT * FROM Commande WHERE id_commande LIKE :rechercheText OR liste_articles LIKE :rechercheText OR fournisseur LIKE :rechercheText");
    ...
    if (query.exec()) {
        ...
        model->setHeaderData(4, Qt::Horizontal, "Liste Articles");
        model->setHeaderData(5, Qt::Horizontal, "Montant Total");
        model->setHeaderData(6, Qt::Horizontal, "Etat Commande");
...
void MainWindow::on_pdf_clicked() {
    ...
    if (model) {
        ...
        content += "<h1 style=\"text-align:center;\">Liste des Commandes (Livré)</h1>";
        ...
        bool hasLivréRows = false;
        for (int row = 0; row < model->rowCount(); ++row) {
            QString etatCommande = model->data(model->index(row, 6)).toString(); // Column 6: Etat Commande
            if (etatCommande.toLower() == "livré") { // Only include rows with "livré"
                hasLivréRows = true;
                content += "<tr>";
        ...
        // If no "livré" rows exist, add a message
        if (!hasLivréRows) {
        ...
        QString fileName = QFileDialog::getSaveFileName(this, "Exporter en PDF", "commandes_livré", "Fichiers PDF (*.pdf)");
...
void MainWindow::on_statistique_clicked()
{
    ...
    QMap<QString, int> statistiques = commandeInstance.statistiquesEtat(); // Replace commandeInstance with your instance

    // Expected states (e.g., from the comboBox)
    QStringList etats = { "En cours", "Livré", "Annulé" };

    if (!statistiques.isEmpty()) {
        QVector<QPair<QString, int>> chartData;

        // Filter the data to display only the relevant states
        for (const QString &etat : etats) {
            int count = statistiques.value(etat, 0); // Default value is 0 if state doesn't exist
        ...
        // Verify if there are any data points
        if (total == 0) {
        ...
        for (const auto &item : chartData) {
            ...
            // Display percentage and state as labels
        ...
        // Display Pie Chart in tableView_3
        ...
        for (const auto &item : chartData) {
            ...
            // Display the state below each bar
            barChartPainter.drawText(
        ...
        // Display Bar Chart in tableView_4
...
float MainWindow::getPrixFromDatabase(const QString &article) {
    ...
    if (query.exec() && query.next()) {
        return query.value(0).toFloat(); // Return the price
    } else {
        return -1; // Indicate that the article wasn't found
    }
}
